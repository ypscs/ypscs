...
public partial class Survey_0000_C : System.Web.UI.Page
{
    ...
    private void SaveData()
    {
        ...
        if (this.PopupMessage1.Length == 0)
        {
            ...
            for (int x = 1; x < 6; x++)
            {
                TextBox tb = Panel_Ans.FindControl("Q334_" + x.ToString()) as TextBox;
                if (!String.IsNullOrEmpty(Q334))
                    Q334 += ">";
    ...
    private void LoadData()
    {
        if (Session["svid"] != null)
        {
            ...
            if (sv.Load(Session["svid"].ToString(), ""))
            {
                ...
                DataRow[] drs = sv.SurveyData.Select("seq>='334' AND seq<='335'");

                for (int x = 0; x < drs.Length; x++)
                {

                    if (drs[x]["seq"].ToString() == "334")
                    {
                        string[] Q334 = drs[x]["ans"].ToString().Split('>');
                        if (Q334.Length > 0)
                        {
                            for (int z = 0; z < Q334.Length; z++)
                            {
                                TextBox tb = Panel_Ans.FindControl("Q334_" + (z + 1).ToString()) as TextBox;
                                if (tb != null)
                                    tb.Text = Q334[z];
                            }
                        }
